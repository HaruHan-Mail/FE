...
const PopuplarTopicSection = () => {
  const topics = ["1️⃣ 하품은 왜 할까?", "2️⃣ 우주 쓰레기", "3️⃣ 하늘로 총을 쏘면 어떻게 될까?", "4️⃣ 딸꾹질은 왜 할까?", "5️⃣ 2차 세계 대전"];
  ...
  useEffect(() => {
    ...
    elements.forEach((el, i) => {
      ...
      if(i === index) 
        animate(el, { opacity: 1, scale: 1, x: 0, zIndex: 3 }, { duration: 0.8 });
      else if(i === prevIndex)
        animate(el, { opacity: 0.5, scale: 0.8, x: -150, zIndex: 2 }, { duration: 0.8 });
      else if(i === nextIndex)
        animate(el, { opacity: 0.5, scale: 0.8, x: 150, zIndex: 2 }, { duration: 0.8 });
      else
        animate(el, { opacity: 0, scale: 0.5, x: 0, zIndex: 1 }, { duration: 0.8 });  
    });
  }, [index]);
